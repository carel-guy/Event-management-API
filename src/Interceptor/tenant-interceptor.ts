...
export class TenantInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (isHttp) {
      ...
      if (!request.user) {
        this.logger.debug(
          'No JWT user found. Attaching mock user context for development.',
        );
        ...
        this.logger.debug(
          `HTTP: Attached mock userContext with tenantId: ${tempTenantId}, userId: ${tempUserId}`,
        );
      } else {
        this.logger.debug(
          'User already attached by AuthenticationGuard. Skipping mock user attachment.',
        );
      }
    } else if (isRpc) {
      ...
      if (!call.user) {
        this.logger.debug(
          'No gRPC user found. Attaching mock user context for development.',
        );
        const metadata: Metadata = context.getArgByIndex(1);
        const tempTenantId =
          metadata.get('x-temp-tenant-id')[0]?.toString() ||
          '60d5ec49f8a3c5a6d8b4567f';
        const tempUserId =
          metadata.get('x-temp-user-id')[0]?.toString() ||
          '60d5ec49f8a3c5a6d8b4567e';

        if (!Types.ObjectId.isValid(tempTenantId)) {
          throw new BadRequestException(
            'Invalid temporary tenant ID format in gRPC metadata.',
          );
        }
        if (!Types.ObjectId.isValid(tempUserId)) {
          throw new BadRequestException(
            'Invalid temporary user ID format in gRPC metadata.',
          );
        }
        ...
        this.logger.debug(
          `gRPC: Attached mock user with tenantId: ${tempTenantId}, userId: ${tempUserId}`,
        );
      } else {
        this.logger.debug(
          'gRPC user already exists. Skipping mock attachment.',
        );
      }
    }
