...
export class KeycloakService {
  ...
  constructor(private configService: VaultConfigService) {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      if (!realmName) {
        this.logger.error(
          'KEYCLOAK_REALM environment variable not set. Realm check will be skipped.',
        );
        return;
      }
      ...
      this.logger.error(
        'Failed to initialize Keycloak service. Please check KEYCLOAK credentials and connection.',
        error.stack,
      );
  ...
  private async getClientInternalId(
    ...
    if (!clientInternalId) {
      this.logger.error(
        `Client '${clientId}' in realm '${realm}' does not have an internal ID.`,
      );
      throw new InternalServerErrorException(
        `Client '${clientId}' is missing an internal ID.`,
      );
    }
  ...
  async createUser(
    ...
    try {
      ...
      this.logger.log(
        `Successfully created user ${newUser.email} in realm ${realm} with ID ${createdUser.id}`,
      );
      return createdUser.id;
    } catch (error) {
      this.logger.error(
        `Failed to create user ${newUser.email} in realm ${realm}`,
        error.stack,
      );
      if (error.response?.status === 409) {
        throw new Error(
          `User with email ${newUser.email} already exists in realm ${realm}.`,
        );
      }
      throw new Error(
        `An unexpected error occurred while creating the user in Keycloak: ${error.message}`,
      );
    }
  }
  ...
  async getUserInfo(userId: string, realm: string) {
    ...
    const realmAccess = await this.adminClient.users.listRealmRoleMappings({
      id: userId,
    });
  ...
  async assignClientRoleToUser(
    ...
    if (!role || !role.id || !role.name) {
      throw new NotFoundException(
        `Role '${roleName}' not found or is invalid for client '${clientId}' in realm '${realm}'.`,
      );
    }
    ...
    try {
      ...
      await this.adminClient.users.addClientRoleMappings({
        id: userId,
        clientUniqueId: clientInternalId,
        roles: [rolePayload],
      });
      this.logger.log(
        `Successfully assigned role '${roleName}' to user '${userId}' in realm '${realm}'.`,
      );
    } catch (error) {
      this.logger.error(
   ...
   */
  async revokeClientRoleFromUser(
    realm: string,
    userId: string,
    roleName: string,
  ): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Successfully revoked role '${roleName}' from user '${userId}' in realm '${realm}'.`,
      );
    } catch (error) {
      this.logger.error(
  ...
  async findUsersByClientRole(
    ...
    try {
      ...
      this.logger.error(
        `Failed to find users with role '${roleName}'.`,
        error.stack,
      );
      throw new InternalServerErrorException(
