...
export class FileReferenceController {
  ...
  @ApiOperation({
    summary: 'Télécharger un fichier et créer une référence de fichier',
  }) // Traduction
  ...
  @ApiResponse({
    status: 201,
    description: 'Fichier et référence créés avec succès.',
    type: FileReference,
  }) // Traduction
  @ApiResponse({
    status: 400,
    description: "Mauvaise requête : Fichier invalide ou données d'entrée.",
  }) // Traduction
  @ApiResponse({
    status: 500,
    description:
      "Erreur interne du serveur : Échec du téléversement ou de l'enregistrement de la référence de fichier.",
  }) // Traduction
  async uploadFile(
    ...
    const uploadedBy: Types.ObjectId | null = req.user.userId
      ? new Types.ObjectId(req.user.userId)
      : null;
    return this.fileReferenceService.uploadAndCreate(
      tenantId,
      uploadedBy,
      file,
  ...
  @ApiResponse({
    status: 200,
    description: 'Liste des références de fichiers.',
    type: [FileReference],
  }) // Traduction
  @ApiQuery({
    name: 'label',
    required: false,
    type: String,
    description: 'Filtrer par libellé de fichier (insensible à la casse).',
  }) // Traduction
  ...
  @ApiResponse({
    status: 200,
    description: 'La référence de fichier trouvée.',
    type: FileReference,
  }) // Traduction
  @ApiNotFoundResponse({
    description:
      "Référence de fichier introuvable ou n'appartient pas à ce locataire.",
  }) // Traduction
  @ApiBadRequestResponse({ description: "Format d'ID invalide." }) // Traduction
  @ApiParam({
    name: 'id',
    description: "L'ID de la référence de fichier.",
    type: String,
  ...
  @ApiInternalServerErrorResponse({
    description:
      'Échec de la récupération du fichier depuis le système de stockage.',
  }) // Traduction
  @ApiParam({
    name: 'id',
    description: "L'ID de la référence de fichier à télécharger.",
    type: String,
    example: '60d5ec49f8a3c5a6d8b4567a',
  }) // Traduction
  async downloadFile(
    ...
    const tenantId: Types.ObjectId = new Types.ObjectId(req.user.tenantId);
    const { stream, mimetype } = await this.fileReferenceService.getFileStream(
      tenantId,
  ...
  @Patch(':id')
  @ApiOperation({
    summary: 'Mettre à jour une référence de fichier existante par ID',
  }) // Traduction
  @ApiResponse({
    status: 200,
    description: 'La référence de fichier mise à jour.',
    type: FileReference,
  }) // Traduction
  @ApiNotFoundResponse({
    description:
  ...
  @ApiBadRequestResponse({
    description: "Données d'entrée ou format d'ID invalide.",
  }) // Traduction
  @ApiParam({
    name: 'id',
    description: "L'ID de la référence de fichier à mettre à jour.",
    type: String,
    example: '60d5ec49f8a3c5a6d8b4567a',
  }) // Traduction
  @ApiBody({
    type: UpdateFileReferenceDto,
    description: 'Données pour mettre à jour la référence de fichier.',
  }) // Traduction
  async update(
    @Param('id') id: string,
    ...
    const tenantId: Types.ObjectId = new Types.ObjectId(req.user.tenantId);
    return this.fileReferenceService.update(
      tenantId,
      new Types.ObjectId(id),
  ...
  @ApiOperation({
    summary:
      'Supprimer une référence de fichier et le fichier associé du stockage par ID',
  }) // Traduction
  @ApiResponse({
    status: 200,
    description: 'Message indiquant la suppression réussie.',
    schema: {
      example: { message: 'Référence de fichier supprimée avec succès.' },
    },
  }) // Traduction
  @ApiNotFoundResponse({
    description:
  ...
  @ApiInternalServerErrorResponse({
    description:
      "Échec de la suppression du fichier du système de stockage ou de l'enregistrement de la base de données.",
  }) // Traduction
  @ApiParam({
    name: 'id',
    description: "L'ID de la référence de fichier à supprimer.",
    type: String,
    example: '60d5ec49f8a3c5a6d8b4567a',
  }) // Traduction
  async remove(
  ...
  @ApiOperation({
    summary: 'Prévisualiser un fichier par ID ou chemin complet',
  })
  @ApiResponse({
    status: 200,
    description: 'Le flux du fichier pour prévisualisation.',
  })
  @ApiNotFoundResponse({
    description: 'Fichier introuvable ou ne peut pas être prévisualisé.',
  })
  @ApiParam({
    name: 'idOrPath',
    description: 'ID de référence ou chemin complet du fichier.',
    type: String,
  })
  async previewFile(@Param('idOrPath') idOrPath: string, @Res() res: Response) {
    try {
      ...
      const { stream, mimetype } =
        await this.fileReferenceService.getFileStreamPreview(identifier);
      ...
      throw new NotFoundException(
        'Fichier introuvable ou ne peut pas être prévisualisé.',
      );
    }
  }
}
